...
class TestRootRouterImports:
    def test_router_exists(self):
        from web.routers.root_router import router
        assert router is not None

    def test_get_ollama_model_list_function_exists(self):
        from web.routers.root_router import get_ollama_model_list
        assert callable(get_ollama_model_list)

    def test_get_comfyui_model_list_function_exists(self):
        from web.routers.root_router import get_comfyui_model_list
        assert callable(get_comfyui_model_list)


class TestGetModels:
    @pytest.mark.asyncio
    @patch('web.routers.root_router.config_service')
    @patch('web.routers.root_router.get_ollama_model_list')
    async def test_get_models_returns_list(self, mock_ollama, mock_config):
        from web.routers.root_router import get_models
    ...
    async def test_get_models_skips_empty_api_key(self, mock_ollama):
        from web.routers.root_router import get_models
        with patch('web.routers.root_router.config_service') as mock_config:
            mock_config.get_config.return_value = {
                'openai': {
                    'url': 'https://api.openai.com',
                    'api_key': '',
...
class TestListTools:
    @pytest.mark.asyncio
    @patch('web.routers.root_router.tool_service')
    @patch('web.routers.root_router.config_service')
    async def test_list_tools_returns_list(self, mock_config, mock_tool_service):
        from web.routers.root_router import list_tools
...
class TestChatSessions:
    @patch('web.routers.root_router.db_service')
    @pytest.mark.asyncio
    async def test_list_chat_sessions(self, mock_db):
        from web.routers.root_router import list_chat_sessions
    ...
    async def test_get_chat_session(self, mock_db):
        from web.routers.root_router import get_chat_session
...
class TestBilling:
    ...
    async def test_get_balance_returns_high_balance(self):
        from web.routers.root_router import get_balance
