...
class TestWorkspaceRouter:
    ...
    def mock_services(self):
        with patch('web.routers.workspace_router.USER_DATA_DIR', tempfile.gettempdir()):
            yield

    def test_get_file_type_image(self):
        from web.routers.workspace_router import get_file_type
...
class TestWorkspaceRouterEndpoints:
    def test_read_file_success(self, tmp_path):
        ...
        from web.routers.workspace_router import WORKSPACE_ROOT, router
        from fastapi import FastAPI
        app = FastAPI()
        app.include_router(router)
        with patch('web.routers.workspace_router.WORKSPACE_ROOT', str(tmp_path)):
    ...
    def test_read_file_not_found(self, tmp_path):
        from web.routers.workspace_router import router
        from fastapi import FastAPI
        app = FastAPI()
        app.include_router(router)
        with patch('web.routers.workspace_router.WORKSPACE_ROOT', str(tmp_path)):
