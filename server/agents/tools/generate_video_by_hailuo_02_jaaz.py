...
from agents.tools.video_generation.video_canvas_utils import send_video_start_notification, process_video_result
...
async def generate_video_by_hailuo_02_jaaz(
    ...
    logger.info("hailuo_start", tool_call_id=tool_call_id, canvas_id=canvas_id, session_id=session_id)
    ...
    try:
        ...
        if input_images and len(input_images) > 0:
            ...
            if processed_image:
                ...
                logger.debug("hailuo_using_input_image", image=first_image)
            else:
                raise ValueError(
        ...
        logger.error("hailuo_error", error=str(e))
        raise e
